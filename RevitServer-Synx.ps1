...
$c.CacheCheckButton.Add_Click({
  ...
  try{$c.StatusText.Text="Глубокая диагностика кэша и AutoSyncLog для $($m.Guid)...";$window.Dispatcher.Invoke([action]{},[Windows.Threading.DispatcherPriority]::Background);$folder=Join-Path (Join-Path $m.InstanceRoot 'SynxBackup') 'ModelDiagnostics';$result=Test-SynxModelDiagnostics -Model $m -LogPaths @($script:analysis.Instance.LogPaths) -ReportDirectory $folder;$dbText=@($result.Cache.DatabaseChecks|ForEach-Object{"$([IO.Path]::GetFileName($_.Path))=$($_.Integrity)"})-join'; ';$last=if($result.Cache.LastWriteTime){([datetime]$result.Cache.LastWriteTime).ToString('dd.MM HH:mm:ss')}else{'нет'};$c.StatusText.Text="Файлов: $($result.Cache.FileCount); проблем: $($result.Cache.IssueCount); SQLite: $dbText; последний файл: $last; строк лога: $($result.LogContextCount). Отчёт: $($result.ReportPath)";Start-Process notepad.exe -ArgumentList ('"'+$result.ReportPath+'"')}catch{$c.StatusText.Text='Ошибка глубокой диагностики модели';Show-Error $_.Exception.Message}
})
