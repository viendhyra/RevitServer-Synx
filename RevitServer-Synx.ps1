...
$names=@('AdminStatusText','InstanceCombo','RefreshButton','SummaryText','LoadNamesButton','OpenLogButton','OpenHistoryButton','OldHostCombo','NewHostText','TestHostButton','MigrateHostButton','LoadHostNamesButton','HostStatusText','ModelsGrid','ErrorsGrid','SelectedText','DiagnosisCard','DiagnosisTitleText','DiagnosisConfidenceText','DiagnosisEvidenceText','DiagnosisActionText','RepairButton','BatchRepairButton','CacheCheckButton','OpenDiagnosticButton','OpenCacheButton','RepairStepText','RepairProgress','RepairDetails','StatusText');$c=@{}
...
$script:busy=$false;$script:batchLimit=5
$window.Add_Closing({param($sender,$e) if($script:busy){$e.Cancel=$true;[void][Windows.MessageBox]::Show($window,'Идёт ремонт. Закрытие окна сейчас оставит службы остановленными, а состояние — рассогласованным. Дождитесь завершения.','RevitServer Synx',[Windows.MessageBoxButton]::OK,[Windows.MessageBoxImage]::Warning)}})
...
$c.InstanceCombo.Add_SelectionChanged({$script:analysis=$null;$script:hostTest=$null;$script:lastDiagnosticPath='';$c.ModelsGrid.ItemsSource=$null;$c.ErrorsGrid.ItemsSource=$null;$c.OldHostCombo.ItemsSource=$null;$c.RepairButton.IsEnabled=$false;$c.BatchRepairButton.IsEnabled=$false;$c.CacheCheckButton.IsEnabled=$false;$c.OpenDiagnosticButton.IsEnabled=$false;$c.OpenCacheButton.IsEnabled=$false;$c.OpenHistoryButton.IsEnabled=$false;$c.MigrateHostButton.IsEnabled=$false;Set-DiagnosisPanel $null '' '' '' ''})
$c.RefreshButton.Add_Click({Refresh-Analysis})
$c.ModelsGrid.Add_SelectionChanged({$m=$c.ModelsGrid.SelectedItem;$script:lastDiagnosticPath='';$c.OpenDiagnosticButton.IsEnabled=$false;if($null-eq$m){$c.SelectedText.Text='Выберите строку слева';$c.RepairButton.IsEnabled=$false;$c.CacheCheckButton.IsEnabled=$false;$c.OpenCacheButton.IsEnabled=$false;Set-DiagnosisPanel $null '' '' '' '';return};$repeatText=if($m.RepeatFailure){'ДА — GUID снова упал после ремонта'}else{'нет'};$c.SelectedText.Text="Имя: $($m.Name)`nПуть модели: $($m.ModelPath)`nGUID: $($m.Guid)`nHost: $($m.HostNode)`nСостояние: $($m.Status)`nПовтор после ремонта: $repeatText`nСлучаев: $($m.IncidentCount); ремонтов: $($m.RepairCount)`nЗависаний: $($m.HangCount)`nКаталог: $($m.CachePath)";Set-DiagnosisPanel $m $m.Diagnosis $m.DiagnosisConfidence $m.DiagnosisEvidence $m.DiagnosisAction;$exists=[bool](Test-Path -LiteralPath $m.CachePath);$c.RepairButton.IsEnabled=[bool]$isAdmin;$c.CacheCheckButton.IsEnabled=$exists;$c.OpenCacheButton.IsEnabled=$exists})
...
$c.CacheCheckButton.Add_Click({
  ...
  try{$c.CacheCheckButton.IsEnabled=$false;$c.StatusText.Text="Определение причины зависания $($m.Guid)...";$window.Dispatcher.Invoke([action]{},[Windows.Threading.DispatcherPriority]::Background);$folder=Join-Path (Join-Path $m.InstanceRoot 'SynxBackup') 'ModelDiagnostics';$result=Test-SynxModelDiagnostics -Model $m -LogPaths @($script:analysis.Instance.LogPaths) -ReportDirectory $folder;$script:lastDiagnosticPath=$result.ReportPath;$m.Diagnosis=$result.Reason;$m.DiagnosisConfidence=$result.Confidence;$m.DiagnosisEvidence=@($result.Evidence)-join"`n";$m.DiagnosisAction=$result.Action;$c.ModelsGrid.Items.Refresh();Set-DiagnosisPanel $m $m.Diagnosis $m.DiagnosisConfidence $m.DiagnosisEvidence $m.DiagnosisAction;$c.OpenDiagnosticButton.IsEnabled=$true;$dbText=@($result.Cache.DatabaseChecks|ForEach-Object{"$([IO.Path]::GetFileName($_.Path))=$($_.Integrity)"})-join'; ';$last=if($result.Cache.LastWriteTime){([datetime]$result.Cache.LastWriteTime).ToString('dd.MM HH:mm:ss')}else{'нет'};$c.StatusText.Text="Причина: $($result.Reason) ($($result.Confidence)). Файлов: $($result.Cache.FileCount); SQLite: $dbText; последний файл: $last. Отчёт: $($result.ReportPath)"}catch{$c.StatusText.Text='Ошибка глубокой диагностики модели';Show-Error $_.Exception.Message}finally{$script:busy=$false;if($null-ne$c.ModelsGrid.SelectedItem){$c.CacheCheckButton.IsEnabled=[bool](Test-Path -LiteralPath $c.ModelsGrid.SelectedItem.CachePath)}}
})
...
$c.BatchRepairButton.Add_Click({
  ...
  $script:busy=$true;$c.RepairProgress.Value=0;$c.RepairDetails.Clear();$c.RepairButton.IsEnabled=$false;$c.BatchRepairButton.IsEnabled=$false;$c.RefreshButton.IsEnabled=$false;$c.InstanceCombo.IsEnabled=$false;$c.ModelsGrid.IsEnabled=$false
  try{
  ...
  finally{$script:busy=$false;$c.RefreshButton.IsEnabled=$true;$c.InstanceCombo.IsEnabled=$true;$c.ModelsGrid.IsEnabled=$true;if($null-ne$c.ModelsGrid.SelectedItem){$c.RepairButton.IsEnabled=[bool]$isAdmin}}
})
$c.RepairButton.Add_Click({
  ...
  try{
    ...
    $script:busy=$true;$c.RepairProgress.Value=0;$c.RepairDetails.Clear();$c.RepairButton.IsEnabled=$false;$c.BatchRepairButton.IsEnabled=$false;$c.RefreshButton.IsEnabled=$false;$c.InstanceCombo.IsEnabled=$false;$c.ModelsGrid.IsEnabled=$false;$c.StatusText.Text='Выполняется ремонт. Не закрывайте окно...'
